...
(ert-deftest fermium-room-folds-account-and-matrix-id-header-details ()
  (with-temp-buffer
    ...
    (should (equal (buffer-substring-no-properties (point) (+ 1 (point)))
                   "▸"))
    (should (eq (get-text-property (point) 'fermium-room-header)
                t))
    ...
    (let ((details-start
      ...
      (should details-start)
      (should (invisible-p details-start))
      (should (string-match-p "Account: @alice:example.org"
                              (buffer-string)))
      (should (string-match-p "Matrix ID: !room:example.org"
                              (buffer-string)))
      (let ((underline-start
             (text-property-any (point-min) (point-max)
                                'fermium-room-header-underline t)))
        (should underline-start)
        (should (< details-start underline-start)))
      (goto-char details-start)
      ...
      (should-not (invisible-p details-start))
